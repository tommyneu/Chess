...
function checkPawnMoves(xPos, yPos) {
	...
	if (board.tiles[xPos][yPos].piece == "PAWN") {
		...
		if (yPos + 2 * dir >= 0 && yPos + 2 * dir <= 7) {
			//checks to see if the two spaces in front of the pawn are empty and that the pawn has not moved yet
		...
		//checks if the hand is already checking things(this could case an infinite loop if not there)
		if (!hand.checking) {
			//checks if all the moves will not case a check on its self or will get out of check
			possibleMoves = checkValidMoves(possibleMoves);
		}
	}
	...
	if (board.tiles[xPos][yPos].piece == "ROOK") {
		...
		for (let dirVal = 0; dirVal < 2; dirVal++) {
			for (let hvVal = 0; hvVal < 2; hvVal++) {
				...
				//creates multipliers for the while loop to keep track of the number of times the loop has gone
				let mult = 1;
				...
				while (pieceCheck) {
					//creates the new X,Y coordinate using the direction of travel and the multiplier
	...
	if (board.tiles[xPos][yPos].piece == "BISHOP") {
		...
		for (let xVal = 0; xVal < 2; xVal++) {
			for (let yVal = 0; yVal < 2; yVal++) {
				...
				//while the bishop has not run into a piece or gone off the board it will keep looping through the spaces
				while (checkPiece) {
			...
			for (let bVal = 0; bVal < 2; bVal++) {
				for (let cVal = 0; cVal < 2; cVal++) {
					...
					while (pieceCheck) {
						...
						let newX = xPos + mult * dirA * bVal * !cVal + mult * dirA * cVal; //cVal is used to determine the horizontal/vertical or diagonal travel
	...
	if (board.tiles[xPos][yPos].piece == "KING") {
		...
		if (!hand.kingCheck) {
			...
			if (board.tiles[xPos][yPos].moved == false) {
				//if it hasn't moved then it checks if the rook to the right of the king has moved
				if (board.tiles[xPos + 3][yPos].moved == false) {
					//if they both haven't moved it then checks if there are no pieces in between the rook and the king
				...
				//if it hasn't moved then it checks if the rook to the left of the king has moved
				if (board.tiles[xPos - 4][yPos].moved == false) {
					//if they both haven't moved it then checks if there are no pieces in between the rook and the king
					if (
						board.tiles[xPos - 1][yPos].piece == null &&
						board.tiles[xPos - 2][yPos].piece == null &&
						board.tiles[xPos - 3][yPos].piece == null
					) {
						//it will the push a special move object with the information for the castle move function
						possibleMoves.push({
							x: xPos,
