...
class Hand {
	constructor() {
		//sets a default x,y location off the board
		this.xPos = -1;
		this.yPos = -1;
		...
		this.draw = function () {
			...
			if (this.highlight) {
				...
				//loops through the moved and colors them blue
				for (let i = 0; i < this.possibleMoves.length; i++) {
					ctx.fillStyle = this.colors[1];
					ctx.fillRect(
						board.Hoffset + this.size * this.possibleMoves[i].newX,
						board.Voffset + this.size * this.possibleMoves[i].newY,
						this.size,
						this.size
					);
				}
