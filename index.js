...
function draw() {
	...
	if (checkMate) {
		...
		//if the game is in a stalemate position it will change the background color to a neutral grey
	} else if (stalemate) {
...
function screenClicked(event) {
	...
	if (mouseX > board.Hoffset && mouseX < board.Hoffset + board.size) {
		if (mouseY > board.Voffset && mouseY < board.Voffset + board.size) {
			...
			for (let i = 0; i < hand.possibleMoves.length; i++) {
				if (hand.possibleMoves[i].newX == tileX && hand.possibleMoves[i].newY == tileY) {
					...
					if (hand.possibleMoves[i].special == "passant") {
						passant(
							hand.possibleMoves[i].x,
							hand.possibleMoves[i].y,
							hand.possibleMoves[i].newX,
							hand.possibleMoves[i].newY,
							hand.possibleMoves[i].dir
						);
						...
						castle(
							...
							hand.possibleMoves[i].y,
							hand.possibleMoves[i].newX,
							hand.possibleMoves[i].rookOldX,
							hand.possibleMoves[i].rookNewX
						);
					} else {
			...
			//if there hasn't been a move made it will reset the hand object and put the new location inside
			if (!moveCheck) {
...
function movePiece(xOld, yOld, xNew, yNew) {
	...
	if (board.tiles[xOld][yOld].piece == "PAWN") {
		...
		if (promotionCheck) {
			...
			//sets up a list of all the available pieces the pawn can switch to
...
//passant moves are moves in which a pawn has moved double the distance but if it had moved the regular distance the other pawn would have been able to take it
...
function passant(xOld, yOld, xNew, yNew, dir) {
	...
	//sets the piece that was taken piece to null
...
function checkCheck() {
	...
	for (let xVal = 0; xVal < 8; xVal++) {
		for (let yVal = 0; yVal < 8; yVal++) {
			//if the piece is the opposite color and there is a piece it will get their moves for possibleMovesCheck
	...
	//it will then loop through the possibleMovesCheck and see if any of the new locations lands on the king
...
//this function when called will check if there is insufficient material to force check on the other player
function insuffMat() {
	//creates all variables needed to increment
	let pawn_queen_rook = [0];
	let bishopL = [0]; //bishops on the same tile matter for one possibility
	let bishopD = [0];
	...
	for (let xVal = 0; xVal < 8; xVal++) {
		for (let yVal = 0; yVal < 8; yVal++) {
			...
			if (piece == "PAWN" || piece == "QUEEN" || piece == "ROOK") {
				...
				bishop[0 + board.tiles[xVal][yVal].pieceColor]++; //(0+) is needed because javascript is weird
	...
	if (pawn_queen_rook == 0) {
		if (bishop[0] + bishop[1] + knight[0] + knight[1] == 0) {
			//if you have two kings and no other pieces is a draw
			stalemate = true;
		} else if (bishop[0] + bishop[1] + knight[0] + knight[1] == 1) {
			//if there is only one minor piece its a draw
			stalemate = true;
		} else if (knight[0] == 2 && knight[1] + bishop[0] + bishop[1] == 0) {
			//if there is two knights on one side or the other its a draw
			stalemate = true;
		} else if (knight[1] == 2 && knight[0] + bishop[0] + bishop[1] == 0) {
			stalemate = true;
		} else if (knight[0] + bishop[0] == 1 && knight[1] + bishop[1] == 1 && bishopD != 2 && bishopL != 2) {
			//if there is one minor piece on either side
			stalemate = true; //and those minor pieces are not bishops on the same color
		} // its a draw
	}
}

//this function checks to see if after the move is done the player is still in check
function checkStillInCheck(move) {
	...
	//it then stores all the relevant data for the two pieces that will move
	...
	if (move.special == "passant") {
		//stores and preforms the special part of the move
	...
	//sets hand.checking to true to stop the moves from calling this function again and creating an infinite loop
	hand.checking = true;

	//loops through all the tiles and gets all the moves from all the moves from the opposite color pieces
	for (let xVal = 0; xVal < 8; xVal++) {
		for (let yVal = 0; yVal < 8; yVal++) {
			//checks if the pieces are the opposite colors and they are there
...
function checkValidMoves(moves) {
	//creates an empty array to hold the valid moves
	let validMoves = [];

	//loops through all the moves and checks if the move will cause a check or gets out of check
