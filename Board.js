...
function Board() {
	//creates some default values for the horizontal and Vertical offsets
	this.Hoffset = 0;
	this.Voffset = 0;
	...
	this.fillTiles = function () {
		...
		for (let xVal = 0; xVal < 8; xVal++) {
			...
			for (let yVal = 0; yVal < 8; yVal++) {
				//it will set the colorVal to default true
				let colorVal = true;

				//if the xVal is divisible by two
				if (!(xVal % 2)) {
					//and the yVal is divisible by two
					if (!(yVal % 2)) {
						//it will not color in that tile
						colorVal = false;
					}
				} else {
					//if the xVal and yVal are not divisible by 2 it will not color in that tile either
					if (yVal % 2) {
						colorVal = false;
					}
				}
				...
				if (yVal == 0 || yVal == 7) {
					...
					//if it is the 7th row it will set the direction to -1(1 by default)
					if (yVal == 7) {
	...
	//the draw method will draw the board and its contents
	this.draw = function () {
